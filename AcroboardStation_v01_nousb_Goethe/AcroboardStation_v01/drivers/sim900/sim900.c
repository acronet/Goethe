...
#include "../goethe/services/config/config.h"
...
#include "devices/PowerSwitch/powerswitch.h"
...
#define LITTLE_DELAY delay_ms(100)
...
static uint8_t sim900_read_data(uint8_t * const szBuf,uint16_t * const lenBuf)
{
	...
	while(l<ec) {
		...
		if(!sim900_wait_data_on_usart(10)) {
			debug_string(NORMAL,PSTR("(sim900_read_data) got timeout waiting for a byte\r\n"),true);
			r=1;
			break;
		}
...
static uint8_t sim900_read_string(char * const szBuf,uint16_t * const lenBuf)
{
	const uint16_t ec = *lenBuf;
...
static uint8_t sim900_cmd_with_read_string(const char * const szCMD,const uint8_t isCMDPGM,char * const szBuf, uint16_t * lenBuf)
{
...
uint16_t sim900_GPRS_check_line(void)
{
	LITTLE_DELAY;
	if(NULL==sim900_cmd_with_retstring(sz_AT,PGM_STRING))
	{
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) SIM900 not responding \r\n"));
		return 1;
	}
	...
	if(szRET!=szCPIN_SIM_READY ) {
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) SIM900 SIM IS PIN LOCKED \r\n"));
		return 2;
	}
	...
	if(strncasecmp_P(szBuf,PSTR("+CREG:"),5)!=0) {
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) device is not answering as it should at CREG command\r\n"));
		return 3;
	}
	if(szBuf[9]!='1') {
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) device NOT correctly registered in the network\r\n"));
		return 4;
	}
	...
	if(strcasecmp_P(szBuf,PSTR("+CGATT: 1"))!=0) {
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) GPRS line not attached\r\n"));
		return 5;
	}
	...
	if(strncasecmp_P(szBuf,PSTR("+CSQ:"),4)!=0) {
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) device is not answering as it should at CSQ command\r\n"));
		return 6;
	}
	...
	if(lq==99) {
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) [WARNING] line quality is undetectable\r\n"));
	} else if(lq<10) {
		debug_string_P(NORMAL,PSTR("(sim900_GPRS_check_line) [WARNING] line quality is less than -93db\r\n"));
	}
...
init_sim:
	...
	for(i=0;i<18;++i) {
		...
		sim900_read_string(szBuf,&l);
		if(strcasecmp_P(szBuf,PSTR("+CGATT: 1"))==0) {
			debug_string_P(NORMAL,PSTR("(SIM900_init) device correctly ATTACHED to the network\r\n"));
			break;
		}
		debug_string_P(NORMAL,PSTR("(SIM900_init) device answered "));
		debug_string(NORMAL,szBuf,RAM_STRING);
		debug_string_P(NORMAL,PSTR(" seems not attached to the network\r\n"));
		debug_string_P(NORMAL,PSTR("(SIM900_init) will check again in 5 second\r\n"));
		delay_ms(5000);
	}
...
uint16_t sim900_http_read(char * const return_value, uint16_t * const rv_len)
{
	char szBuf[64];
...
uint16_t sim900_tcp_init( const char * const service_APN, const uint8_t isSZPGM )
{
	LITTLE_DELAY;
	if(NULL==sim900_cmd_with_retstring(sz_AT,PGM_STRING))
	{
		debug_string_P(NORMAL,PSTR("(sim900_tcp_init) SIM900 not responding \r\n"));
		return 1;
	}
	...
	for (uint8_t i=0;i<2;i++)
	{
		...
		if(szRet == NULL) {
			debug_string_P(NORMAL,PSTR("(sim900_tcp_init) timeout waiting for AT+CSTT command, retry\r\n"));
			continue;
		}
		if(szRet!=sz_OK) {
			debug_string_P(NORMAL,PSTR("(sim900_tcp_init) AT+CSTT command not OK\r\n"));
			return 2;
		}
		break;
	}
...
uint16_t sim900_tcp_connect( const char * const service, const uint8_t isSZServicePGM, const char * port, const uint8_t isSZPortPGM)
{
	LITTLE_DELAY;
	if(NULL==sim900_cmd_with_retstring(sz_AT,PGM_STRING))
	{
		debug_string_P(NORMAL,PSTR("(sim900_tcp_connect) SIM900 not responding \r\n"));
		return 1;
	}
	
	if(0!=sim900_GPRS_check_line()) {
		debug_string_P(NORMAL,PSTR("(sim900_tcp_connect) GPRS line not OK \r\n"));
		return 2;
	}
	...
	{
		debug_string_P(NORMAL,PSTR("(sim900_tcp_connect) AT+CIICR not able to connect \r\n"));
		return 3;
	}
	...
	for(uint8_t i = 0;i<2;++i) {
		...
		uint8_t r = sim900_read_string(szBuf,&l);
		if(r==0) {
			if ( strcasecmp_P(szBuf,PSTR("0.0.0.0"))==0 ) {
				debug_string_P(NORMAL,PSTR("(sim900_tcp_connect) device has no valid IP address\r\n"));
				return 4;
			} else {
				debug_string(NORMAL,PSTR("(sim900_tcp_connect) device IP address : "),PGM_STRING);
				debug_string(NORMAL,szBuf,RAM_STRING);
	...
	sim900_put_string(PSTR("AT+CIPSTART=\"TCP\",\""),PGM_STRING);
	...
	{
		debug_string_P(NORMAL,PSTR("(sim900_tcp_connect) AT+CIPSTART not able to connect \r\n"));
		return 4;
	}
	
	if(NULL==sim900_wait_string(PSTR("CONNECT OK"),PGM_STRING))
	{
		debug_string_P(NORMAL,PSTR("(sim900_tcp_connect) AT+CIPSTART not able to connect -2-\r\n"));
		return 5;
	}
...
uint16_t sim900_tcp_send(	const char * const pBufSend, 
							const uint8_t isBufPGM, 
							const uint16_t dimBufSend, 
							char * const pBufAnswer, 
							const uint16_t * pdimBufAnsw)
{
	LITTLE_DELAY;
	if(NULL==sim900_cmd_with_retstring(sz_AT,PGM_STRING))
	{
	...
	char szBuf[32];
	sprintf_P(szBuf,PSTR("AT+CIPSEND=%d\r\nAA"),dimBufSend);


	LITTLE_DELAY;
	sim900_put_string(szBuf,RAM_STRING);
	if(NULL==sim900_wait_string(PSTR(">"),PGM_STRING)) 
	{
		debug_string_P(NORMAL,PSTR("(sim900_tcp_send) error with CIPSEND\r\n"));
		return 2;
	}

	LITTLE_DELAY;
	sim900_put_data(pBufSend,dimBufSend,isBufPGM);
	...
	return sim900_read_data(pBufAnswer,pdimBufAnsw);
}

uint16_t sim900_http_get_prepare(	const char * const get_URL,
									const uint8_t isURLPGM )
{
	LITTLE_DELAY;
	if(NULL==sim900_cmd_with_retstring(sz_AT,PGM_STRING))
	{
		debug_string_P(NORMAL,PSTR("(http_get_prepare) SIM900 not responding \r\n"));
		return 1;
	}
	...
	{
		debug_string_P(NORMAL,PSTR("(http_get_prepare) AT+HTTPINIT not OK\r\n"));
		return 1;
	}
	...
	{
		debug_string_P(NORMAL,PSTR("(http_get_prepare) AT+HTTPPARA=\"CID\" not OK\r\n"));
		return 1;
	}
	...
	{
		debug_string_P(NORMAL,PSTR("(http_get_prepare) HTTPACTION didn't return OK\r\n"));
		return 1;
	}
	
	if(NULL == sim900_wait_string(PSTR("+HTTPACTION:"),PGM_STRING))
	{
		debug_string_P(NORMAL,PSTR("(http_get_prepare) didn't got +httpaction aborting\r\n"));
		return 1;
	}
	...
	if(1==r) {
		debug_string_P(NORMAL,PSTR("(http_get_prepare) got timeout waiting for httpaction response\r\n"));
		return 1;
	}

	debug_string_P(NORMAL,PSTR("(http_get_prepare) httpaction returned: "));
	debug_string(NORMAL,szBuf,RAM_STRING);
	debug_string_P(NORMAL,szCRLF);
	
	if(szBuf[2]!='2' || szBuf[3]!='0' || szBuf[4]!='0') {
		//HTTPACTION didn't return OK
		debug_string_P(NORMAL,PSTR("(http_get_prepare) httpget response was not OK\r\n"));
		return 1;
	}
...
uint16_t sim900_http_get(	const char * const get_URL, 
							...
							uint16_t * const rv_len )
{

	uint16_t r = sim900_http_get_prepare(get_URL,isURLPGM);
	if(r!=0)
	{
		return r;
	}
	
	return sim900_http_read(return_value,rv_len);
}
							...
							const uint8_t isPostPGM )
{
	...
	if(NULL==sim900_wait_string(PSTR("DOWNLOAD"),PGM_STRING))
	{
		return 1;
	}
...
// We need to know the return code of the post
	...
	debug_string(NORMAL,PSTR("(sim900_http_post) httpaction returned: "),PGM_STRING);
	debug_string(NORMAL,szBuf,RAM_STRING);
