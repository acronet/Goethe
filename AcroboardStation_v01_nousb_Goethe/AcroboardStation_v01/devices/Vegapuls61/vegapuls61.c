...
#define LG_NUMSTATS				23 // Number of reading involved in the getStats function


static uint16_t g_measureBuffer[MEASUREBUFFERSIZE];
volatile static uint16_t g_index = 0;
...
void vp61_getStats(LG_VP61_STATS * const ps)
{
	...
	measureBuf.maxVal = g_measureBuffer[start_index];
	measureBuf.minVal = g_measureBuffer[g_index];
