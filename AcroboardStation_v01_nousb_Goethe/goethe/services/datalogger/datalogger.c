...
/////////////////////////////////////////////////////////////
// SENSORCODE: include sensors' header files
...
typedef struct DL_LOG_ITEM {
	...
	//////////////////////////////////////////////////////////////////////
	// SENSORCODE: insert here sensor type for Datalogger_Log_ITEM
	uint16_t vBat;
	LG_VP61_STATS vegapulsStats;
	uint16_t tempMCU;
	//////////////////////////////////////////////////////////////////////
	uint8_t flags;	
...
uint16_t datalogger_init( void )
{
	...
	//while(1) {delay_ms(200);gpio_toggle_pin(STATUS_LED_PIN);}
	...
	//calcute a value that gets 
	//overflows at a rate of 100Hz approx
	...
	dl_snapshot_init(-1);
	//////////////////////////////////////////////////////////////////////////
 	// SENSORCODE: Insert here sensors' init functions
 	voltmeter_init();
	vp61_init();
	//////////////////////////////////////////////////////////////////////////
...
uint16_t dl_task_sync_time( void )
{
	...
	uint32_t tOLD = g_timePrev;
	uint32_t tNEW = strtoul(&buf[0],NULL,10);
	...
	calendar_timestamp_to_date(tOLD,&adate);
...
static uint16_t dl_cmd_fw_update(const char * pPara)
{
	datalogger_stop();
	...
	while(1) {delay_ms(100);gpio_toggle_pin(STATUS_LED_PIN);}
	return 0;
}
...
uint8_t send_data_with_get( DL_SEND_PARAMS * const pPara )
{
	...
	/////////////////////////////////////////////////////////////////////
	// SENSORCODE: Modify output string
	
	
	//RAINGAUGE_STATS * rs = &ds.raingauge_stats[0];

	//const uint16_t mxs0   = rs->maxSlope;
	snprintf_P(szBuf+l1,BUFFER_SIZE-l1,PSTR("&TIME=%lu&B=%u&adcL=%u"),
	ds.data_timestamp,
	ds.vBat,
	ds.vegapulsStats.voltageVal
	);
...
uint16_t send_data_with_post( DL_SEND_PARAMS * const pPara )
{
	...
	while(iter<QUEUE_LEN) {
		...
		PSTR("#TIME=%lu&B=%u&V_L=%u&V_Lnp=%u&V_Lmed=%u&V_Lmax=%u&V_Lmin=%u&T_MCU=%u") ,
		ds.data_timestamp ,
		ds.vBat ,
		ds.vegapulsStats.voltageVal ,
		ds.vegapulsStats.voltageValNoPeaks ,
		ds.vegapulsStats.medianVal ,
		ds.vegapulsStats.maxVal ,
		ds.vegapulsStats.minVal ,
		ds.tempMCU
		);
...
static void dl_snapshot_iter_dec(volatile uint8_t * const idx)
{
	if( *idx == 0 ) {
		(*idx)=(SNAPSHOT_SIZE-1);
		} else {
...
static void dl_snapshot_iter_inc(volatile uint8_t * const idx)
{
...
static void  dl_snapshot_init(const uint32_t ts)
{
	...
	volatile DL_LOG_ITEM * const p_item = &g_data_snapshot[0];
...
static uint8_t  dl_snapshot_make(const uint32_t ts)
{
	volatile DL_LOG_ITEM * p_item = &g_data_snapshot[g_data_snapshot_end];
	
	//////////////////////////////////////////////////////////////////////////////////////////////////
	// SENSORCODE: Insert here get_stats for sensor measurements
	volatile LG_VP61_STATS * const ls = &p_item->vegapulsStats;
	vp61_getStats(ls);
...
uint16_t dl_task_store_data( void )
{
	...
	while(idx!=g_data_snapshot_end) {
		volatile DL_LOG_ITEM * const pd = &g_data_snapshot[idx];
...
void datalogger_run(void)
{
	while(1) {
		//////////////////////////////////////////////////////////
		// SENSORCODE: Insert here get_value code. This code is called every tick (5 sec.)
		vp61_triggerReading();
		//////////////////////////////////////////////////////////
...
int dl_mqtt_read_packet(int timeout)
{

	return 0;
}

int dl_mqtt_send(void* socket_info, const void* buf, unsigned int count)
{
	sim900_tcp_send(PSTR("CIAO"),PGM_STRING,4,0,0);
	return 0;
}
...
static void dl_test_now(void)
{
	...
	//while(1)
	//{
	//sprintf_P(szBuf,PSTR("F_CPU: %lu DFLL: 0x%02X%02X CALA: 0x%02X  CALB: 0x%02X\r\n"),F_CPU,DFLLRC32M.COMP2,DFLLRC32M.COMP1,DFLLRC32M.CALA,DFLLRC32M.CALB);
	//debug_string(NORMAL,szBuf,RAM_STRING);
	//
	//gpio_toggle_pin(STATUS_LED_PIN);
	////cpu_delay_ms(5000,F_CPU);
	//delay_s(5);
	//
	//}
	
	//sim900_init();
	//sim900_GPRS_check_line();
	//sim900_tcp_init(PSTR("internet"),PGM_STRING);
	//
	//mqtt_broker_handle_t broker;
	//
	//mqtt_init(&broker, "client-id");
	//
	//sim900_tcp_connect(PSTR("130.251.104.241"),PGM_STRING,PSTR("1883"),PGM_STRING);
	//
	//broker.send = dl_mqtt_send;
	//
	//mqtt_connect(&broker);
	//
	////	int packet_length = dl_mqtt_read_packet(1);
	//sim900_tcp_send(PSTR("CIAO\r\n"),PGM_STRING,6,0,0);
